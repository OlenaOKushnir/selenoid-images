    ...
    _.extend(AvastWRC,
        {
            ...
            init : function(callerID){
                ...
                AvastWRC.CONFIG.DEBUG_MODE = AvastWRC.getWindowStorage("DEBUG_MODE") || DEBUG_MODE;
            ...
            getUrlInfo: function (urlDetails, callback, notvisited) {
                ...
                for(var i=0, j=urls.length; i<j; i++) {
                    ...
                    if (cUrl && AvastWRC.bs.checkUrl(cUrl)) {
                        if (AvastWRC.bal.sp){
                            var cached = this.Cache.get(cUrl);
            ...
            onTabRemoved : function(tabId, removeInfo) {
                ...
                {
                    (AvastWRC.Utils.Burger) ? AvastWRC.Utils.Burger.initBurger(true/*sendAll*/) : console.log("no burger lib");
                }
            ...
            onSendHeaders: function(details) {
                if(details.type === "main_frame") {
                    ...
                    if (AvastWRC.bs.ciuvoASdetector) {
                        console.log("DETECTOR: AvastWRC.bs.ciuvoASdetector available");
...
(function(AvastWRC, _, Q, PROTO) {
  ...
  AvastWRC.local = AvastWRC.local || {
    ...
    setSourceId: function () {
        if(AvastWRC.CONFIG.LOCAL_ENABLED){
            new AvastWRC.Query.Avast({
                type: AvastWRC.gpb.All.LocalServerRequest.CommandType.GET_BCU_DISTRIBUTION_ID,
                callback: function (r) {
                    console.log(`Success GET_BCU_DISTRIBUTION_ID:`, r);
                    let decoded = _.map(r.values_.result, function(v) { return v ? PROTO.decodeUTF8(v) : v; } );
                    if(decoded.length <= 0)return;
                    let result = JSON.parse(decoded) || {};
                    console.log(`Success GET_BCU_DISTRIBUTION_ID decoded:`, result);
                    let sourceId = result[AvastWRC.Utils.getBrowserInfo().getCurrentBrowserShortName()] || "";
                    if(sourceId == "" || !result.av || result.av == "")return;
                    if(AvastWRC.bal.brandingType == AvastWRC.BRANDING_TYPE_AVAST && result.av.toLowerCase() === "avast"
                        || AvastWRC.bal.brandingType == AvastWRC.BRANDING_TYPE_AVG && result.av.toLowerCase() === "avg"){
...
(function(AvastWRC) {
	...
	AvastWRC.Utils.getClientInfo = function(campaignId){
		...
		clientInfo.install_time = AvastWRC.PROTO.I64.fromNumber(AvastWRC.CONFIG.InstallTimestamp);
	...
	AvastWRC.Utils.resolveImageUrl = function(primaryImage = "", secondaryImage = "", subcategory = 0){
		...
		return {
			barImage: (secondaryImage !== "" && secondaryImage.indexOf("rfid") != -1) ? getImageUrl(imagesMap["rfid"]) : (!imagesMap[subcategory]) ? getImageUrl(imagesMap["0"]) : getImageUrl(imagesMap[subcategory]),
