(function(_, AvastWRC, EventEmitter) {
	...
	Burger.initGlobalData = function(_data, _sendAll){
		...
		Burger.Collection.events.SEND_INTERVAL_TIME =  AvastWRC.getWindowStorage("send_burger_interval") || 300000;

		var burgerConfigs = (AvastWRC.Shepherd)? AvastWRC.Shepherd.getBurgerSendInfo() : null
		if(burgerConfigs){
			// localstorage for test || shepherd || default 
			Burger.Collection.events.BATCH_MAX_MESSAGES = AvastWRC.getWindowStorage("max_burger_messages") || burgerConfigs.BATCH_MAX_MESSAGES || 20;
			Burger.Collection.events.SEND_INTERVAL_TIME =  AvastWRC.getWindowStorage("send_burger_interval") || burgerConfigs.SEND_INTERVAL_TIME  || 300000;
		}
	...
	*/
	...
	Burger.Build = {
		...
		identityMessage : function(){
			...
			if(Burger.GLOBAL.IDENTITY_GUID === ""){
...
(function(_, AvastWRC) {
	...
	_.extend(Burger.Build,{
		...
		userReportsMessage: function (eventDetails) {			
			...
			}else if(eventDetails.type == "SHOP"){
		...
		clientInfoMessage: function(clientInfo){
			...
			_clientInfo.install_time = clientInfo.install_time;
		...
		buildCouponMessage: function (offer, offerCategory){
			var voucher = new Burger.Gpb.Blob.PickedOffer.Voucher;
			if(offerCategory == "VOUCHER"){
				voucher["title"] = offer["label"] || "";
				voucher["url"] = offer["url"] || "";
				voucher["affiliate"] = offer["affiliate"] || "";
				voucher["value"] = offer["value"] || 0;
				voucher["expire_date"] = offer["expire_date"] || "";
				voucher["code"] = offer["coupon_code"] || "";
				voucher["text"] = offer["coupon_text"] || "";
				voucher["free_shipping"] = offer["free_shipping"] || 0;
				voucher["type"] = offer["type"] || 0;
				voucher["provider_id"] = offer["provider_id"] || "";
			}
			return voucher;
		},
		...
		buildAccommodationMessage: function (offer, offerCategory) {
			...
			if(offerCategory == "ACCOMMODATION"){	
				accommodation.offer["title"] = offer["label"] || "";
				accommodation.offer["price"] = offer["price"] || 0;
				accommodation.offer["formatted_price"] = offer["fprice"] || "";
				...
				accommodation["stars"] = offer["stars"] || 0;
				accommodation["additional_fees"] = offer["additional_fees"] || 0;
				accommodation["price_netto"] = offer["price_netto"] || 0;
				accommodation["saving"] = offer["saving"] || "";
				accommodation["type"] = offer["type"] || 0;
				accommodation["stars_precise"] = offer["stars_precise"] || 0;
				accommodation["city"] = offer["city"] || "";
				accommodation.offer["provider_id"] = offer["provider_id"] || "";
			}
		...
		buildRedirectMessage: function (offer, offerCategory) {
			...
			if(offerCategory == "REDIRECT"){	
				redirect["title"] = offer["label"] || "";
				redirect["url"] = offer["url"] || "";
				redirect["formatted_price"] = offer["fprice"] || "";
				...
				redirect["provider_redirect_id"] = offer["provider_redirect_id"] || "";
			}
			return redirect;
		},
		...
		pickedOfferMessage: function (offer, offerQuery, offerCategory){
			...
			pickedOffer.voucher = this.buildCouponMessage(offer, offerCategory);
		...
		blobMessage : function(eventDetails){
			...
			if (blobEventInfo.secondLevel == 2  && offer != null && offerCategory != ""){
				...
				blob.voucher = this.buildCouponMessage(offer, offerCategory);
			}
			...
			if(clientInfo != null){
				if(blobEventInfo.secondLevel == 1){
